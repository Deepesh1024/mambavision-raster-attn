...
def verify_setup():
    print('---')
    print('SEC 1: SETUP AND VERIFICATION')
    print('---')
...
def download_document_images(device):
    ...
    transform = transforms.Compose([transforms.Resize((768, 1024)), transforms.ToTensor(), transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    urls = ['https://fki.tic.heia-fr.ch/static/img/a01-122-02.jpg', 'https://fki.tic.heia-fr.ch/static/img/a01-122-02-00.jpg']
    images = []
    labels = []
        ...
        for i, label in enumerate(labels):
            ...
            for vname in ['square8', 'raster_s2', 'raster_s4']:
                if embeddings[vname] is not None and i < len(embeddings[vname]):
                    cos_sim = F.cosine_similarity(base_emb.flatten().unsqueeze(0), embeddings[vname][i].flatten().unsqueeze(0)).item()
                    row[vname] = cos_sim
...
def benchmark_variant(model, vname, device, resolution, warmup=5, iterations=20):
    ...
    try:
        ...
        peak_vram = torch.cuda.max_memory_allocated() / 1000000000.0
        del x
        torch.cuda.empty_cache()
...
def run_benchmarks(base_model, device, use_deepcopy):
    print('---')
    print('SEC 4: BENCHMARK')
    print('---')
    resolutions = [(512, 512), (1024, 1024), (768, 1024)]
    variants = ['baseline', 'square8', 'raster_s2', 'raster_s4']
    variant_labels = {'baseline': 'Baseline (14x14)', 'square8': 'Square 8x8', 'raster_s2': 'Raster s=2', 'raster_s4': 'Raster s=4'}
    results = []
    for vname in variants:
        ...
        try:
            ...
            for res in resolutions:
                results.append({'variant': vname, 'resolution': f'{res[0]}x{res[1]}', 'error': f'Creation failed: {str(e)[:200]}'})
            continue
        for res in resolutions:
            print(f'  Resolution {res[0]}x{res[1]}...', end=' ', flush=True)
            r = benchmark_variant(model, vname, device, res)
            entry = {'variant': vname, 'variant_label': variant_labels[vname], 'resolution': f'{res[0]}x{res[1]}'}
            if 'error' in r:
                entry['error'] = r['error']
